// wallet-polygon-network-utils.js

const WalletPolygonUtils = (function () {
  ...
  async function deriveAddressFromSeedHex(seedHex, provider) {
    ...
    if (provider.assetName?.toLowerCase() !== 'polygon') {
      throw new Error('Error deriving address: Wrong asset passed for Polygon: ' + JSON.stringify(provider, null, 2));
    }
  ...
  async function estimateFee(transaction) {
    return await WalletEthersUtils.estimateFee(transaction, 'polygon');
  }
