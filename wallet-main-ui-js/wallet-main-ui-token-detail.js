// wallet-main-ui-token-detail.js

(function (window) {
  let _initialized = false;
  let _currentAssetKey = null;

  const detailPanel = document.getElementById("tokenDetailPanel");
  const receiveBtn = document.getElementById("tokenReceiveButton");
  const sendBtn = document.getElementById("tokenSendButton");
  const receiveAddressContainer = document.getElementById("tokenReceiveAddressContainer");
  ...
  async function refreshCurrentAssetBalance() {
    ...
    try {
      ...
      const current = WalletPersistence.walletManager.activeWallet.derivedTokens;
      current[_currentAssetKey] = updatedAsset;
      WalletPersistence.walletManager.activeWallet.derivedTokens = current;

      WalletBusyModal.hide();

      WalletPageLevelUiStatusLines.updateStatus([ ...statusLines, `✅ Updated ${asset.tokenSymbol} balance` ]);
      showTokenDetails(_currentAssetKey);
    } catch (err) {
      WalletBusyModal.hide();
  ...
  function showTokenDetails(key) {

    _currentAssetKey = key || _currentAssetKey;

    const derived = WalletPersistence.walletManager.activeWallet.derivedTokens?.[_currentAssetKey] || {};
    ...
    const titleElem = document.getElementById("tokenDetailTitle");
    titleElem.innerHTML = `
      <div class="cryptoDetailTitleFlex">
      <button id="tokenDetailRefreshBtn" class="cryptoDetailRefreshBtn" title="Refresh balance">🔄</button>
    ...
    const refreshBtn = document.getElementById("tokenDetailRefreshBtn");
    if (refreshBtn) {
    ...
    let infoBlock = document.getElementById("tokenExtraInfo");
    if (!infoBlock) {
      infoBlock = document.createElement("div");
      infoBlock.id = "tokenExtraInfo";
  ...
  function handleSendClick() {
    ...
    WalletTokenSend.showSendPanel(_currentAssetKey);
  }
