// wallet-main-ui-stablecoin-send.js

const WalletStableCoinSend = (function () {
  ...
  function cacheDom() {
    dom.panelTitle = document.getElementById("stableCoinSendPanelTitle");
    dom.toAddress = document.getElementById("stableCoinSendToAddress");
    dom.amount = document.getElementById("stableCoinSendAmount");
    dom.fee = document.getElementById("stableCoinSendEstimatedFee");
    dom.maxAmountLink = document.getElementById("stableCoinSendMaxAmountLink");
    dom.feeHelpIcon = document.getElementById("stableCoinSendFeeHelpIcon");
    dom.continueBtn = document.getElementById("stableCoinSendContinueButton");
    dom.sendBtn = document.getElementById("stableCoinSendSendButton");
    dom.resetBtn = document.getElementById("stableCoinSendResetButton");
  }
  ...
  function showSendPanel(assetKey) {
    ...
    const asset = WalletPersistence.walletManager.activeWallet.derivedStableCoins?.[assetKey] || {};
    ...
    if (dom.panelTitle && asset?.tokenSymbol) {
      ...
      dom.panelTitle.textContent = `Send ${asset.assetName} (${asset.tokenSymbol}) [${net}]`;
    } else {
    ...
    if (dom.maxAmountLink && _currentMaxAmount !== null) {
      dom.maxAmountLink.innerHTML = `max <a href="#" id="stableCoinSendMaxAmountClick">${_currentMaxAmount}</a>`;
      const maxClickEl = document.getElementById("stableCoinSendMaxAmountClick");
      if (maxClickEl) {
    ...
    validateForm();
    WalletUiPanel.show('stableCoinSendPanel');
  }
  ...
  async function fetchAndHandleBalance() {
    ...
    try {
      ...
      await WalletMultiTokenUtils.getBalanceForTokenAsset(transaction, 'stablecoins');
      WalletBusyModal.hide();

      const latestBalance = WalletPersistence.walletManager.activeWallet.derivedStableCoins?.[_currentAssetKey]?.balance;

      if (latestBalance !== null && cachedBalance !== null && latestBalance < cachedBalance) {
        _currentMaxAmount = latestBalance;

        if (dom.maxAmountLink) {
          dom.maxAmountLink.innerHTML = `max <a href="#" id="stableCoinsSendMaxAmountClick">${_currentMaxAmount}</a>`;
          const maxClickEl = document.getElementById("stableCoinsSendMaxAmountClick");
          if (maxClickEl) {
            maxClickEl.addEventListener("click", e => {
              e.preventDefault();
              dom.amount.value = _currentMaxAmount;
  ...
  async function onSendClicked() {
    ...
    try {
      ...
      // Give browser a chance to paint the modal/spinner
      await new Promise(r => requestAnimationFrame(r));
      await WalletMultiTokenUtils.getBalanceForTokenAsset(_transaction, 'stablecoins');
      WalletBusyModal.hide();

      const persistedBalance = WalletPersistence.walletManager.activeWallet.derivedStableCoins?.[_transaction.assetKey]?.balance;
      const statusLines = [
